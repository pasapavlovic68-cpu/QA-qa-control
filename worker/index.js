...
2. СКОРОСТЬ ОТВЕТА:
   ...
   В диалоге есть таймстампы в формате (ДД.ММ.ГГГГ ЧЧ:ММ) или (ЧЧ:ММ) — используй их.
   Найди самое раннее сообщение CUSTOMER и самое раннее сообщение SALE, вычисли разницу.
   Если первый ответ > 10 минут — нарушение severity high, укажи сколько минут было потрачено.
   Если таймстампы есть — обязательно укажи response_time_minutes (число).
   Если таймстампов нет — null.
...

{
  ...
  "violations": [
    { "rule": string, "quote": string, "severity": "critical" | "high", "explanation": string }
  ],
  "mistakes": [
...
funnel_check — анализ прохождения воронки:
  ...
  response_time_minutes — время от первого сообщения CUSTOMER до первого ответа SALE в минутах (число). Если таймстампов нет — null. Если диалог начинает SALE — 0.
